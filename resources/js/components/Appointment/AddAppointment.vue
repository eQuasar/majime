                ...
                <div class="card main_booking">
                                ...
                                <tab-content title="Find User & Choose Pet" :selected="true">
                                    ...
                                    <div class="autosuggest-container" style="position: relative;clear: both;" v-else>
                                        ...
                                        >
                                            <div slot-scope="{suggestion}" style="display: flex; align-items: center;">
                                                <div style="{ display: 'flex', color: 'navyblue'}">{{suggestion.item.user.phone}} - {{suggestion.item.user.name}} - {{suggestion.item.address}}</div>
                                            </div>
                                        </vue-autosuggest>
                                    ...
                                    <div class="choose_pet" v-show="isHidden">
                                        <b-form-group label="Select Pet" v-slot="{ ariaDescribedby }">
                                            <b-form-radio-group v-model="pet_select" id="pet-radios-btn" :aria-describedby="ariaDescribedby" button-variant="outline-primary" size="lg" name="radio-btn-outline" buttons>
                    ...
                    <div class="card-footer text-center">
                        <div class="btn-group" role="group">
                            ...
                            <button @click="onSubmit" v-if="currentTab === totalTabs - 2" class="btn btn-success">Submit</button>
                        </div>
                    </div>
                </div>
            </div>
...
import petaggresive from '../../api/petaggresive.js';
    export default {
        ...
        pet_dob_data(){
          let d = "01";
          let m = this.birthday_month;
          let y = this.birthday_year;
          this.pet_dob = y+m+d;
          let mdate = y+'-'+m+'-'+d;
         ...
         var month_age = Math.floor(day_age/30);   
          this.pet_actual_age = year_age+ " years " + month_age + " months " + day_age + " days";
        },
        onChangeBreed(value){
         breed.getBreed(value)
           .then(( response ) => {
              if(this.pet_name == ''){
                this.pet_name = response.data.data[0].name;
              }
    ...
    show_data(index)
      {
        ...
        this.pet_dob = this.pets[index].dob_simple;
        this.birthday_year = this.pet_dob.slice(0,4);
        //alert(this.pet_dob.slice(5,7));
        this.birthday_month = this.pet_dob.slice(5,7);

          let mdate = this.pet_dob;
              var dobYear = parseInt(mdate.substring(0,4), 10);  
            var dobMonth = parseInt(mdate.substring(5,7), 10);  
            var dobDate = parseInt(mdate.substring(8,10), 10);  
    ...
    getImgpet(pet) {
        ...
        return images;
            ...
            onSubmit() {
                ...
                formData.append("pet_id", this.pet_select);
                formData.append("service_id", this.bath);
                formData.append("additional_service", this.additional_service);
                formData.append("vehicle", this.vehicle);
                formData.append("date", this.date);
