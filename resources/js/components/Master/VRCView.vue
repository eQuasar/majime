      ...
      <div class="header_inner">
        <b-row>
          <b-col xl="4" lg="4" md="4">
            <h3><strong>Vendor Rate Card View Detail</strong></h3><br/>
          </b-col>
      ...
      <div class="card list-appointments space-bottom">
        ...
        sort-icon-left :filter-included-fields="filterOn" :filter="filter" :fields="fields" :per-page="perPage" :current-page="currentPage" show-empty>
              ...
              <template v-slot:cell(action)="row">
                ...
                <router-link :to="{ name: 'OrderProfile', params: { id: (row.item.id).toString() }}"><b-icon icon="eye-fill" aria-hidden="true"></b-icon></router-link>
    ...
    data() 
    {
      return {
        ...
        fields: [
          ...
          {
            key: 'codper',
            label: 'Cash on delivery Percent',
            sortable: true
          },
          {
            key: 'after500gm',
...
computed: {
  ...
  methods: {
    ...
    onSubmit(event) {
      ...
      let formData = new FormData();
      formData.append("date_from", this.date_from);view_Transaction
      formData.append("date_to", this.date_to);
      
      viewTran.ViewTras(formData)
          .then((response) => {
