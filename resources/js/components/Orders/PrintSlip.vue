<template>
  <b-container fluid> 
    ...
    <div class="header_inner">
        <h3><strong>Packed Orders</strong></h3><br/>
      </div>
    </div>
      ...
      sort-icon-left :filter-included-fields="filterOn" :filter="filter" :fields="fields" :per-page="perPage" :current-page="currentPage" show-empty>
          ...
          <template v-slot:cell(sr)="row">
            ...
            <template v-slot:cell(action)="row">
                <b-link @click="printOrderSlip(row.item.oid)"><b-icon icon="key" aria-hidden="true" data-toggle="tooltip" title="Print Order Slip"></b-icon></b-link>
            </template>
