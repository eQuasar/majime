    ...
    <b-container fluid>
        <div class="header_title">
            <div class="header_inner">
                <h2><strong>DTO-Intransit</strong></h2>
            </div>
        </div>
        <div class="clear">&nbsp;</div>
        <b-row>
...
computed: {
  ...
  getVidz()
    ...
    getstatus(vid) 
    {
       ...
       Returndto.get_Statusdto(vid,'dtointransit')
          .then(( response ) => {
            if(response.data)
            {
              this.items=response.data;
 ...
 pendingdownload()
      {
            ...
            formData.append("status",'dtointransit')
            order.Pending_downloadsheet(formData)
             .then((response) => {
              console.log(response.data[0]);
                ...
                XLSX.writeFile(wb,'DtoIntransit_orders.xlsx')
          })
          .catch((error) => {
              console.log(error);
              if (error.response.status == 422) {
