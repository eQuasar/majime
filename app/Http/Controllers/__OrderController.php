...
class OrderController extends Controller
{
	...
	public function order_details(Request $request)
	{
		...
		// $orders = DB::table('orders')->join('billings', 'orders.oid', '=', 'billings.order_id')->join('line_items', 'orders.oid', '=', 'line_items.order_id')->select('orders.*', 'billings.*', DB::raw("count(line_items.order_id) as count"))->groupBy('orders.oid')->get();
		 // $obj=Orders::join('billings','orders.oid','=','billings.order_id')->get(['orders.*', 'billings.*']);
	     // ->join('line_items','line_items.order_id','=', 'orders.oid')->get(['orders.*', 'billings.*','line_items.quantity']);
        //$obj= orders::all();
